...
async function lookupClientAndCoach(clientId: number) {
  const coach = await queryOne<{ id: number; name: string }>(
    `SELECT u.id, u.name FROM client_info ci
     JOIN users u ON u.id = ci.coach_id
     WHERE ci.user_id = $1`,
    [clientId]
  );
...
export async function notifyJournalHearted(opts: {
  clientId: number;
  date: string;
}) {
  try {
    const ctx = await lookupClientAndCoach(opts.clientId);
    if (!ctx) return;
    const { coach, client } = ctx;
    ...
    const pushPromise = sendPushToUser(opts.clientId, {
      title: `${coachFirst} hearted your reflection`,
      body: `Your reflection for ${opts.date} got some love.`,
      url: path,
    }).catch((err) => console.error('journal-heart push failed:', err));
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      emailPromise = resend.emails
        .send({
          from: process.env.REMINDER_FROM_EMAIL || 'Win the Day <onboarding@resend.dev>',
          to: client.email,
          subject: `${coachFirst} hearted your reflection`,
          html: `
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px;">
              <h2 style="color: #1A1714; font-size: 20px; margin-bottom: 8px;">A little love from ${coachFirst}</h2>
              <p style="color: #5A4F45; font-size: 15px; line-height: 1.6;">
                ${coachFirst} read your reflection for ${opts.date} and hearted it. ❤️
              </p>
              <a href="${link}"
                 style="display: inline-block; margin-top: 16px; padding: 12px 24px; background: #B5705A; color: white; text-decoration: none; border-radius: 999px; font-weight: 500; font-size: 14px;">
                Open journal
        ...
        .catch((err) => console.error('journal-heart email failed:', err));
    }

    await Promise.all([pushPromise, emailPromise]);
  } catch (err) {
    console.error('notifyJournalHearted failed:', err);
  }
}

export async function notifyCompletionHearted(opts: {
  clientId: number;
  date: string;
}) {
  try {
    ...
    const pushPromise = sendPushToUser(opts.clientId, {
      title: `${coachFirst} congratulated you`,
      body: 'You won the day. ❤️',
      url: path,
    }).catch((err) => console.error('completion-heart push failed:', err));
    ...
    if (process.env.RESEND_API_KEY) {
      ...
      emailPromise = resend.emails
        .send({
          from: process.env.REMINDER_FROM_EMAIL || 'Win the Day <onboarding@resend.dev>',
          to: client.email,
          subject: `${coachFirst} congratulated you on winning the day`,
          html: `
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px;">
              <h2 style="color: #1A1714; font-size: 20px; margin-bottom: 8px;">You won the day</h2>
              <p style="color: #5A4F45; font-size: 15px; line-height: 1.6;">
                ${coachFirst} saw that you finished every commitment on ${opts.date} and sent some love. ❤️
              </p>
              <a href="${link}"
                 style="display: inline-block; margin-top: 16px; padding: 12px 24px; background: #B5705A; color: white; text-decoration: none; border-radius: 999px; font-weight: 500; font-size: 14px;">
                Open Today
        ...
        .catch((err) => console.error('completion-heart email failed:', err));
    }

    await Promise.all([pushPromise, emailPromise]);
  } catch (err) {
    console.error('notifyCompletionHearted failed:', err);
  }
}
